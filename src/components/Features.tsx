        ...
        <div className="mt-12 grid gap-px border border-border bg-border md:grid-cols-3">
          {features.map((feature) => {
            ...
            return (
              <article
                key={feature.tech}
                className="flex flex-col bg-surface p-8 transition-colors hover:bg-background"
              >
                <span className="flex h-10 w-10 items-center justify-center border border-steel/30 bg-steel/5">
                  <Icon className="h-5 w-5 text-steel" aria-hidden="true" />
                </span>
                <h3 className="mt-6 font-heading text-lg font-semibold text-foreground">
                  {feature.tech}
                </h3>
                <div className="mt-3 h-px w-10 bg-steel/40" />
                <p className="mt-3 text-sm leading-6 text-muted">{feature.value}</p>
              </article>
            );
          })}
        </div>
